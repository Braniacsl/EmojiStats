...
# This is a simplified check; a more robust library might be better but sticking to requirements
def is_unicode_emoji(char):
    # Basic check covering common emoji ranges
    # See: https://unicode.org/Public/emoji/15.0/emoji-test.txt
    code = ord(char)
    # Emoticons, Dingbats, Transport and Map Symbols, Misc Symbols and Pictographs, etc.
    return (
        ...
        # Add more ranges if needed, e.g., regional indicators
    )
...
def setup(bot: discord.ext.commands.Bot):
    ...
    async def on_message(message: discord.Message):
        ...
        for name, emoji_id in custom_emojis_found:
            # Construct the full emoji string <:name:id> or <a:name:id>
            # We need to know if it was animated. The regex doesn't capture the 'a'.
            # Let's re-search to get the full match.
        ...
        # Iterate through characters to find Unicode emojis
        # This might count components of composite emojis separately; refinement might be needed
        unicode_emojis_found = set()
        for char in message.content:
            if is_unicode_emoji(char):
                # Check if it's part of a variation sequence or ZWJ sequence (basic check)
                # A more sophisticated grapheme cluster analysis would be better.
                # For simplicity, log individual emoji characters found.
        ...
        if message.stickers:
            for sticker in message.stickers:
                ...
                # Use sticker ID for tracking, store name as well
        ...
        # If using commands.Bot, this is usually handled automatically unless you specifically block it.
        # If using discord.Client, you might need: await bot.process_commands(message)
